...
fastq_samples = glob_wildcards("./FASTQ/{sample}.fastq").sample
...
## fastq_to_fastqc: quality control checks on raw sequence data
## .fastq files must be located in a directory named /FASTQ
## output will be directed to a directory named /FASTQC/{sample}
...
# I need to adapt all the following rules to the outputs in rnaseq_paired
